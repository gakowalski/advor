...
HTREEITEM insert_new_node(TV_INSERTSTRUCT *nodeinfo)
{
	...
	{	result = (HTREEITEM)SendDlgItemMessage(hDlgNetInfo,25500,TVM_INSERTITEM,0,(LPARAM)nodeinfo);
		if(result)	break;
		if(i & 4)
...
{	if(!hItem) return;
	...
	{	if(!streams->_base.marked_for_close)
		{	uint32_t raddr=tor_addr_to_ipv4n(&streams->_base.addr);
			...
			{	tvinsw.hInsertAfter=(HTREEITEM)SendDlgItemMessageW(hDlgNetInfo,25500,TVM_INSERTITEMW,0,(LPARAM)&tvinsw);
...
{	LPARAM lParam;
	...
	{	LangAppendMenu(hMenu,MF_STRING|MF_UNCHECKED|MF_ENABLED,25003,LANG_NETINFO_DESTROY_CIRCUIT);
		...
		{	HMENU hMenu2 = CreatePopupMenu();
			...
			{	if(max_dirtiness)
				{	if(circ->timestamp_dirty > (get_time(NULL) + 0x10000000ULL))	CheckMenuItem(hMenu2,25025,MF_BYCOMMAND|MF_CHECKED);
					else if(circ->timestamp_dirty < (uint64_t)(get_time(NULL)-max_dirtiness))	CheckMenuItem(hMenu2,25027,MF_BYCOMMAND|MF_CHECKED);
