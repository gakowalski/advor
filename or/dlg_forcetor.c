...
{	LV_ITEMW lvitw;
	...
	lvitw.iItem=0;
	lvitw.iSubItem=0;
	lvitw.mask=LVIF_PARAM|LVIF_TEXT;
	lvitw.cchTextMax=99;
	lvitw.pszText=tmpMenu;
	int i=0;
	while(i<MAX_PID_LIST)
	{	lvitw.lParam=0;
		if(SendDlgItemMessageW(hDlgForceTor,12400,LVM_GETITEMW,0,(LPARAM)&lvitw)==0) break;
		if(lvitw.lParam)
		{	if(SendDlgItemMessageW(hDlgForceTor,12400,LVM_GETITEMSTATE,lvitw.iItem,LVIS_STATEIMAGEMASK)&8192)
			{	char *utfstr = get_utf(tmpMenu);
				tor_snprintf(tmpMenu1,200,"%s (PID: %d)",utfstr,(int)lvitw.lParam);
				tor_free(utfstr);
				LangAppendMenuStr(hMenu3,MF_STRING,22000+i,tmpMenu1);pidlist[i]=lvitw.lParam;i++;
			}
		}
		lvitw.iItem++;
	}
...
{	if(uMsg==WM_INITDIALOG)
	...
	{	if(LOWORD(wParam)==12001)
		...
		{	lvitw.iItem = SendDlgItemMessage(hDlg,12400,LVM_GETNEXTITEM,-1,LVNI_ALL|LVNI_SELECTED);
			...
			if(wParam == -1)	LangMessageBox(hMainDialog,get_lang_str(LANG_MB_NO_SELECTION),LANG_MB_ERROR,MB_OK);
		}
