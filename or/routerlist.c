...
{	digestmap_t *pending;
	...
	{	int found = 0;
		...
		SMARTLIST_FOREACH(cl->certs, authority_cert_t *, cert,
		{	if(now < cert->expires)		/* It's not expired, and we weren't looking for something to verify a consensus with. Call it done. */
			{	download_status_reset(&cl->dl_status);
				found = 1;
				break;
			}
...
void
extrainfo_free(extrainfo_t *extrainfo,char *c,int n)
{
  if (!extrainfo)
    return;
...
static void
_extrainfo_free(void *e,char *c,int n)
{
