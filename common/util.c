...
void * __stdcall safe_malloc(int size)
{	int memsize = size;
	DWORD *next_mem_buf;
...
#ifdef DEBUG_MALLOC
void *_tor_malloc(size_t size DMALLOC_PARAMS,char *file,int line)
#else
...
#ifdef DEBUG_MALLOC
void _tor_free_(void *p,char *file,int line)
{
	if(p)
	{
		while(!TryEnterCriticalSection(&allocCriticalSection))
 ...
 */
#ifdef DEBUG_MALLOC
void *_tor_malloc_zero(size_t size DMALLOC_PARAMS,char *c,int n)
{
 ...
 */
void *_tor_realloc(void *ptr, size_t size DMALLOC_PARAMS,char *file,int line)
#else
...
// allocate a buffer that is not cached to pagefile.sys
void *tor_safe_alloc(int count)
{	return VirtualAlloc(NULL,count,MEM_COMMIT,PAGE_READWRITE|PAGE_NOCACHE);
}

void tor_safe_free(void *buffer)
 ...
 */
#ifdef DEBUG_MALLOC
char *_tor_strdup(const char *s DMALLOC_PARAMS,char *file,int line)
#else
 ...
 */
#ifdef DEBUG_MALLOC
char *_tor_strndup(const char *s, size_t n DMALLOC_PARAMS,char *file,int line)
#else
...
#ifdef DEBUG_MALLOC
void *_tor_malloc_roundup(size_t *sizep DMALLOC_PARAMS,char *c,int n)
{
...
int strfind(char *s1,char *s2,int s1_len)
{	int i = 0,j = (s1_len?s1_len:strlen(s1)) -strlen(s2);
	if(j < 0)	return -1;
	while(i <= j)
...
int strcasefind(char *s1,char *s2,int s1_len)
{	int i = 0,j = (s1_len?s1_len:strlen(s1)) -strlen(s2);
	if(j < 0)	return -1;
	while(i <= j)
