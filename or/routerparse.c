...
#ifdef DEBUG_MALLOC
static void
_free_duplicate_routerstatus_entry(void *e,char *c,int n)
{
	...
	{	SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, _tok)
		...
		{	SMARTLIST_FOREACH_BEGIN(tokens, directory_token_t *, _tok)
			{	const char *id_hexdigest;
				...
				{	if(s->alg == alg && tor_memeq(id_digest, s->identity_digest, DIGEST_LEN) && tor_memeq(sk_digest, s->signing_key_digest, DIGEST_LEN))
						is_duplicate = 1;
				});
				if(is_duplicate)
...
sort_version_list(smartlist_t *versions, int remove_duplicates)
{
  ...
  if (remove_duplicates)
    smartlist_uniq(versions, _compare_tor_version_str_ptr, _tor_free_);
}
