...
{	uint32_t raddr=geoip_reverse(addr);
	...
	{	raddr=get_router_sel();
		...
		{	raddr=geoip_reverse(raddr);
			country = geoip_get_country_by_ip(raddr);
			tor_snprintf(newname,200,"%s [exit(%s%s): %d.%d.%d.%d]%s",exename,geoip_get_country_name(country&0xff),country>0xff?"*":"",raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,(addr==-1)?"":" - no exit");
		}
		else if(get_country_sel()<0x1ff)	tor_snprintf(newname,200,"%s [exit(%s)]%s",exename,geoip_get_country_name(get_country_sel()),(addr==-1)?"":" - no exit");
...
HTREEITEM addTreeItem(HTREEITEM hParent,char *name,DWORD lParam,int tree_idx)
{	if(!name)
	...
	hParent = (HTREEITEM)SendDlgItemMessageW(hMainDialog,200,TVM_INSERTITEMW,0,(LPARAM)&tvins);
	if(tree_idx!=-1)	pages[tree_idx] = hParent;
	tor_free(tvins.item.pszText);
	return hParent;
}

void setTreeItem(HTREEITEM hItem,char *name)
{	if(name)
...
int __stdcall dlgfunc(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	{	hMainDialog=hDlg;
		...
		get_winver();
		tor_snprintf(newname,200,"%s  %s by Albu Cristian, 2009-2016",exename,advtor_ver);
