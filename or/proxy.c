...
{	unsigned int len,i,j,http_hdr;
	...
	{	case 5: /* socks5 */
			...
			{	unsigned char ulen = buf->head->data[1],ulen1;
...
{	if(conn->_base.mode == CONNECTION_MODE_OTHER)	return;
	buf_t *buf=conn->_base.inbuf;
	chunk_t *dest;
	char *startaddr,*next,*newhdrs;
	int i = conn->_base.processed_from_inbuf;
				...
				{	case 4:		// socks4
					...
					case 5:		// socks5
						...
						{	unsigned char nummethods = (unsigned char)*(&conn->_base.unprocessed[0]+1);
							...
							conn->_base.processed_from_inbuf += 2u + nummethods,buf;
